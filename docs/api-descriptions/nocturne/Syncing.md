...
- Glooko XT (the former Diabnext service, reached by setting the Glooko connector's server to `XT`) exposes one read call over its whole logbook rather than per-type feeds, so a sync reads a single window — from the older of the glucose and treatment resume points to now, a week at a time — and fans every record out into the enabled types. Pump-mode spans (closed loop, attempting, boost, ease-off, and manual inferred between them), safety alerts, consumable changes and the pump-settings profile exist only in its CSV export, which the same sync reads a fortnight at a time when those types are enabled. Its sign-in is a code emailed to the patient, completed once in the connector settings; the resulting year-long token is what every sync presents.
