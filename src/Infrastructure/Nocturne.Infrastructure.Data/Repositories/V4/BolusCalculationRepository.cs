...
public class BolusCalculationRepository
{
    private readonly NocturneDbContext _context;
    private readonly ILogger<BolusCalculationRepository> _logger;

    public BolusCalculationRepository(NocturneDbContext context, ILogger<BolusCalculationRepository> logger)
    {
        _context = context;
        _logger = logger;
    }
    ...
    {
        var query = _context.BolusCalculations.AsQueryable();
    ...
    public async Task<BolusCalculation?> GetByLegacyIdAsync(string legacyId, CancellationToken ct = default)
    {
        var entity = await _context.BolusCalculations.FirstOrDefaultAsync(e => e.LegacyId == legacyId, ct);
        return entity is null ? null : BolusCalculationMapper.ToDomainModel(entity);
    }

    public async Task<BolusCalculation> CreateAsync(BolusCalculation model, CancellationToken ct = default)
    {
    ...
    public async Task<BolusCalculation> UpdateAsync(Guid id, BolusCalculation model, CancellationToken ct = default)
    {
        var entity = await _context.BolusCalculations.FindAsync([id], ct)
    ...
    public async Task<IEnumerable<BolusCalculation>> GetByCorrelationIdAsync(Guid correlationId, CancellationToken ct = default)
    {
        var entities = await _context.BolusCalculations
