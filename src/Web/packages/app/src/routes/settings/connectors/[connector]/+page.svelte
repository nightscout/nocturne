<script lang="ts">
	...
	async function loadData() {
		...
		try {
			...
			connectorInfo = servicesOverview?.availableConnectors?.find(
			...
			// Load schema and existing configuration in parallel
			const [schemaResult, configResult] = await Promise.all([
				getConnectorSchema(connectorInfo.name!),
				getConnectorConfiguration(connectorInfo.name!).catch(() => null),
			]);
			...
			error = e instanceof Error ? e.message : 'Failed to load connector configuration';
		} finally {
			isLoading = false;
		}
	}
	...
	async function handleSaveConfiguration(config: Record<string, unknown>) {
		...
		if (result.success) {
			...
			saveMessage = { type: 'error', text: result.error || 'Failed to save configuration' };
		}
	...
	async function handleSaveSecrets(newSecrets: Record<string, string>) {
		...
		if (result.success) {
			...
			saveMessage = { type: 'error', text: result.error || 'Failed to save credentials' };
		}
	...
	async function handleToggleActive(isActive: boolean) {
		...
		if (result.success) {
			...
			saveMessage = { type: 'error', text: result.error || 'Failed to update connector state' };
		}
	...
	async function handleDelete() {
		...
		const result = await deleteConnectorConfiguration(connectorInfo.name);
		...
		if (result.success) {
			...
			saveMessage = { type: 'error', text: result.error || 'Failed to delete configuration' };
		}
	}
...
<div class="container mx-auto p-6 max-w-3xl space-y-6">
	<!-- Back Navigation -->
	<div>
		<Button variant="ghost" size="sm" href="/settings/services" class="gap-1 -ml-2 mb-4">
	...
	{:else if connectorInfo && schema}
		...
		<!-- Save Message -->
		{#if saveMessage}
			<Card class={saveMessage.type === 'error' ? 'border-destructive' : 'border-green-500'}>
	...
	<AlertDialog.Content>
		<AlertDialog.Header>
			...
			<AlertDialog.Description>
				This will permanently delete all configuration and credentials for this connector.
				The connector will stop syncing data. This action cannot be undone.
			</AlertDialog.Description>
		</AlertDialog.Header>
		<AlertDialog.Footer>
