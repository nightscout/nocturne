...
<div class="@container container mx-auto max-w-4xl p-3 @md:p-6 space-y-6">
  ...
  {:else if servicesOverview}
    ...
    <Card {@attach coachmark({
      ...
      <CardContent>
        {#if !googleHealth?.connected && (!servicesOverview.activeDataSources || servicesOverview.activeDataSources.length === 0)}
        ...
        {:else}
          <div class="space-y-3">
            ...
            {/if}
            {#each servicesOverview.activeDataSources.filter((source) => source.sourceType !== "google-health" && source.deviceId !== "google-health") as source (source.id)}
              {@const matchingUploader = getMatchingUploader(source)}
