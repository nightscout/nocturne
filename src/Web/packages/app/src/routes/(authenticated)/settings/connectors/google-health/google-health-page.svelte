<script lang="ts">
  ...
  async function loadPreview() {
    ...
    inventoryBusy = true;
    try { preview = await previewPersonalGoogleHealth(); }
  ...
  async function refresh(loadInventory = true) {
    operation = "status"; status = await getPersonalGoogleHealth().run(); clientId = status.clientId ?? "";
    callbackUrl = status.callbackUrl || `${location.origin}/personal/google/callback`;
  ...
  async function saveChanges(sync: boolean) {
    ...
    try {
      if (sync && selected.length > 0) {
        operation = "sync";
        status = await syncPersonalGoogleHealth();
  ...
  async function pollSync() {
    ...
    statusPolling = true;
    try {
      const updated = await getPersonalGoogleHealth().run();
