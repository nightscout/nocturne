<script lang="ts">
  ...
  const statistics = $derived<BatteryStatistics[]>(
    batteryResource.current?.statistics ?? []
  );
  const cycles = $derived<ChargeCycle[]>(batteryResource.current?.cycles ?? []);
  const readings = $derived<DeviceStatus[]>(
    batteryResource.current?.readings ?? []
  );
        ...
        <CardContent class="pt-6">
          <div class="text-center py-8">
            ...
            <p class="text-sm text-muted-foreground mt-2">
              Battery data is collected from devices that report uploader
              status. Make sure your CGM uploader app is sending device status
              data.
            </p>
          </div>
        </CardContent>
                      ...
                      {#if cycle.dischargeDurationMinutes}
                        <div class="text-xs text-muted-foreground">
                          Lasted: {formatDuration(
                            cycle.dischargeDurationMinutes
                          )}
