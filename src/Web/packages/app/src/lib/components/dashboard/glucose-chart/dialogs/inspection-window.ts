 ...
 */
export function inspectionSearchWindow(timestamp: Date): { from: string; to: string } {
  const centerMs = timestamp.getTime();
  return {
    from: new Date(centerMs - SEARCH_WINDOW_MS).toISOString(),
    to: new Date(centerMs + SEARCH_WINDOW_MS).toISOString(),
  };
}
