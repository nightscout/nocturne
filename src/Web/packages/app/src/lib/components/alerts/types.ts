...
/** Narrows an arbitrary value to a member of a string-valued enum. */
export function isStringEnumMember<E extends Record<string, string>>(
	enumObject: E,
	value: unknown,
): value is E[keyof E] {
	return (
...
/** The generated enum member for a condition kind; the two are the same strings. */
export function toAlertConditionType(kind: ConditionKind): AlertConditionType | undefined {
	return Object.values(AlertConditionType).find((member) => member === kind);
}
...
const TREND_BUCKETS: readonly TrendBucket[] = [
	"falling_fast",
	"falling",
	"flat",
	"rising",
...
export function isTrendBucket(value: unknown): value is TrendBucket {
	return TREND_BUCKETS.some((bucket) => bucket === value);
}
...
export interface ConditionNode {
	...
	sleep_session_active?: SleepSessionActiveCondition;
	tracker_age?: TrackerAgeCondition;
}

/** Builds a node of <paramref name="kind"/> carrying its payload under the matching key. */
export function withPayload<K extends ConditionKind>(
	kind: K,
	payload: NonNullable<ConditionNode[K]>,
): ConditionNode {
	const node: ConditionNode = { type: kind };
	Object.assign(node, { [kind]: payload });
	return node;
}
	...
	switch (kind) {
		...
		case "sleep_session_active":
			return {
				...
				sleep_session_active: { is_active: true },
			};
		case "tracker_age":
			return {
				type: "tracker_age",
...
export function nodeFromApiEnvelope(envelope: unknown): ConditionNode | null {
	...
	const payload: unknown = Reflect.get(envelope, kind);
...
/**
 * The one place a condition read off the wire is adopted as typed. The
 * generated client types rule condition JSON as `any`, and the editor takes it
 * as-is. Rejecting a rule it cannot fully validate would load the default
 * condition in its place, and saving would then overwrite the stored one.
 */
function adoptWireNode(raw: { type: string; [key: string]: unknown }): ConditionNode {
	// eslint-disable-next-line @typescript-eslint/consistent-type-assertions -- trust boundary, see above
...
export function nodeFromApi(
	...
	const node = adoptWireNode({ type: kind, _uid: newUid(), [kind]: params });
...
): { conditionType: ConditionKind; conditionParams: unknown } | null {
	if (!node) return null;
	const params = node[node.type];
...
export function buildBody(state: RuleEditorState) {
	...
	return {
		...
		conditionType: api ? toAlertConditionType(api.conditionType) : undefined,
...
function parseSnoozeConditions(snooze: unknown): ConditionNode[] {
	if (!isRecord(snooze)) return [];
	const raw = snooze.conditions;
...
export function parseRule(r: AlertRuleResponse | null): RuleEditorState {
	...
	return {
		...
		severity: r.severity ?? AlertRuleSeverity.Warning,
