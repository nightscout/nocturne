...
/**
 * Editor-local condition payload for the `sleep_session_active` leaf. The
 * backend record lives in `Nocturne.Core.Models.AlertModels`
 * (<c>SleepSessionActiveCondition</c>); this hand-written mirror exists because
 * the generated API client is regenerated by CI and does not yet expose the
 * type. Follow-up: drop this once the generated client carries
 * <c>SleepSessionActiveCondition</c> and import it from `$api-clients` alongside
 * the sibling condition types above.
 */
export interface SleepSessionActiveCondition {
	is_active: boolean;
}
 ...
 *
 * `sleep_session_active` is spliced in locally: the generated
 * <c>AlertConditionType</c> enum does not yet include it (CI regenerates the
 * client). Remove the union addition once the generated enum carries the value.
 */
export type ConditionKind = `${AlertConditionType}` | "sleep_session_active";
