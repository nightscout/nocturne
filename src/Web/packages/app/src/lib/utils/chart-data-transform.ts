...
/**
 * Transform raw NSwag DashboardChartData into the shape consumed by chart
 * components. Converts mills timestamps to Date objects and resolves chart
 * colors to CSS variables. Glucose values are kept in mg/dL — unit conversion
 * to mmol/L is applied at the display layer (Y-axis labels, tooltips) via bg()
 * on the client.
 *
