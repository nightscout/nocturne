...
export const getEstimate = query(async () => {
  const apiClient = getRequestEvent().locals.apiClient;
  try {
    return await apiClient.reservoir.getEstimate();
  } catch (err) {
    ...
    console.error('Error in reservoir.getEstimate:', err);
    const e = err as any;
    ...
    throw error(500, message ?? 'Failed to get estimate');
  }
});
