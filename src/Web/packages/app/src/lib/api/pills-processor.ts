...
/**
 * Profile represents therapy settings/profile data Mirrors Nightscout profile
 * structure
 */
...
/**
 * Strongly typed interfaces for OpenAPS nested data structures. The backend
 * types these as `any` but we know their structure from Nightscout.
 */
...
/**
 * Strongly typed interface for Loop enacted data. The backend types this as
 * `any` but we know its structure.
 */
...
/**
 * Helper to safely access OpenAPS IOB data which can be an array or single
 * object
 */
...
export function processIOB(
  ...
  const recentStatuses = deviceStatuses
    .filter((ds) => {
      const mills =
        ds.mills ?? (ds.created_at ? new Date(ds.created_at).getTime() : 0);
      return mills <= futureMills && mills >= recentMills;
    })
    .sort((a, b) => {
      const aMills =
        a.mills ?? (a.created_at ? new Date(a.created_at).getTime() : 0);
      const bMills =
        b.mills ?? (b.created_at ? new Date(b.created_at).getTime() : 0);
  ...
  for (const status of recentStatuses) {
    ...
    if (loopIob && typeof loopIob.iob === "number") {
      const mills = loopIob.timestamp
        ? new Date(loopIob.timestamp).getTime()
        : (status.mills ?? now);
      iobFromDevice = {
        iob: loopIob.iob,
        source: "Loop",
    ...
    if (pumpIob) {
      const iobValue =
        typeof pumpIob === "number"
          ? pumpIob
          : (pumpIob.iob ?? pumpIob.bolusiob);
...
export function processCOB(
  ...
  for (const status of recentStatuses) {
    ...
    if (loopCob && typeof loopCob.cob === "number") {
      const mills = loopCob.timestamp
        ? new Date(loopCob.timestamp).getTime()
        : (status.mills ?? now);
      cobFromDevice = {
        cob: loopCob.cob,
        source: "Loop",
    ...
    if (openaps) {
      const suggested = openaps.suggested as
        | OpenApsSuggestedEnacted
        | undefined;
      ...
      if (suggested && enacted) {
        const suggestedTime = suggested.timestamp
          ? new Date(suggested.timestamp).getTime()
          : 0;
        const enactedTime = enacted.timestamp
          ? new Date(enacted.timestamp).getTime()
          : 0;
        ...
        lastCOB = enacted.COB ?? null;
        lastMoment = enacted.timestamp
          ? new Date(enacted.timestamp).getTime()
          : null;
      } else if (suggested) {
        lastCOB = suggested.COB ?? null;
        lastMoment = suggested.timestamp
          ? new Date(suggested.timestamp).getTime()
          : null;
      }
  ...
  if (lastCarbs) {
    ...
    const estimatedCOB = Math.max(
      0,
      (lastCarbs.carbs ?? 0) - hoursSinceCarbs * 30
    );
...
export function processCAGE(
  ...
  const siteChanges = deviceEvents
    .filter(
      (d) =>
        (d.eventType === DeviceEventType.SiteChange ||
          d.eventType === DeviceEventType.CannulaChange) &&
        (d.mills ?? 0) <= now
    )
...
export function processSAGE(
  ...
  const sensorEvents = deviceEvents
    .filter(
      (d) =>
        (d.eventType === DeviceEventType.SensorStart ||
          d.eventType === DeviceEventType.SensorChange) &&
        (d.mills ?? 0) <= now
    )
    .sort((a, b) => (b.mills ?? 0) - (a.mills ?? 0));
 ...
 */
function getScheduledBasalRate(
  profile: Profile | null,
  now: number
): { rate: number; profileName: string | null } {
  if (!profile) {
  ...
  const profileData = profile.store[activeProfileName] as
    | { basal?: TimeValueEntry[] }
    | undefined;
  ...
  const sortedBasal = [...profileData.basal]
    .filter(
      (entry): entry is { time: string; value: number } =>
...
export function processBasal(
  ...
  const { rate: scheduledBasal, profileName } = getScheduledBasalRate(
    profile,
    now
  );
  ...
  if (openaps?.enacted) {
    ...
    if (enacted.rate !== undefined && enacted.duration !== undefined) {
      ...
      const enactedTimestamp = enacted.timestamp
        ? new Date(enacted.timestamp).getTime()
        : now;
...
export function processLoop(
  ...
  // Process Loop data
  if (loopData) {
    const loopTimestamp = loopData.timestamp
      ? new Date(loopData.timestamp).getTime()
      : now;
