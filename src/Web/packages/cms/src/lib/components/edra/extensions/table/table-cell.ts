...
import { getCellsInColumn, isRowSelected, selectRow } from './utils.ts';
...
const showGripAt = (target: EventTarget | null) => {
	...
	const index = cell.parentElement instanceof HTMLTableRowElement ? cell.parentElement.rowIndex : undefined;
	const grips = table.querySelectorAll<HTMLAnchorElement>('a.grip-row');
	grips.forEach((g, idx) => {
		if (idx === index) g.classList.add('show-row-grip');
		else g.classList.remove('show-row-grip');
	});
	const wrapper = table.closest('.tableWrapper');
	if (wrapper) {
		const lastIndex = table.rows.length ? table.rows.length - 1 : -1;
		if (index === lastIndex) wrapper.classList.add('last-row-hover');
