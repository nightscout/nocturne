...
The API client pipeline output (`src/Web/packages/app/src/lib/api/generated/` and `src/Web/packages/app/src/lib/api/api-client.generated.ts`) is **gitignored**, not tracked. It's regenerated by the API build (Aspire startup, `dotnet build`), and every CI consumer (docker-publish, sdk-publish) regenerates its own copy. A fresh clone has no client until the first API build — run `aspire start` or `dotnet build src/API/Nocturne.API/Nocturne.API.csproj` before frontend typechecking.

A few other generated sets ARE tracked in git so they're browsable on GitHub; local changes to them are just noise. One-time setup after clone:

```bash
cd src/Web && pnpm run hide-generated   # set --skip-worktree on all tracked-generated files
cd src/Web && pnpm run unhide-generated # undo (e.g. before staging a deliberate change)
```

Files covered:
- **Wuchale translations** (`src/Web/locales/`) — `.po` sources synced by CI on string changes; compiled JS outputs regenerated at build time
- **Docker Compose bundles** (`deploy/docker-compose/`, `deploy/portainer/`) — regenerated by `scripts/publish-release.cs` before tagging a release
- **V4 SDK OpenAPI spec** (`sdk/openapi-v4.json`) — filtered SDK surface (V4 + OAuth endpoints) regenerated and committed by CI on API/model changes (`sync-sdk-spec.yml`); the sdk/*/out/ SDK build outputs stay gitignored
